...
@testset "kepler_init" begin
    ...
    mass = 1.0
    period = 1.5
    global elements = ones(Float64,6)
    ...
    ntime = 10000
    time = range(t0,step=t0+period,length=ntime)
    ...
    for i=1:ntime
        global x,v = kepler_init(timebig[i],massbig,big.(elements))
    ...
    println("Total area: ",Atot," ",pi*sqrt(1.0 - ecc^2)*semi^2," ratio: ",Atot/pi/semi^2/sqrt(1.0 - ecc^2))
    #using PyPlot
    #plot(time,dAdt)
