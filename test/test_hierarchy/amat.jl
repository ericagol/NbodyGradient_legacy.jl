...
################################################################################
function amat_kep_jac(h::Array{T},elements::Array{T,2},IC::Array{Any,1},rkepler::Array{T},rdotkepler::Array{T},jac_init::Array{Float64,2}) where {T <: Real}
    ...
    for i in 1:length(h[:,1])
        ...
        if iter2 == 1
            mc2 = mass[index2[1]]
            h[i,index2] .= 1.0
            dh[i,index2,index2] .= -1.0/mc2
        elseif iter2 > 1
            mc2 = summass(mass,index2,iter2)
            h[i,index2] = mass[index2]/mc2
            dh[i,index2,1:n_nbody] .= -mass[index2]/mc2^2
        end
        r,rdot = kepler_init(t0,mc1+mc2,elements[i,2:7],jac_init)
        for j=1:3
          rkepler[i,j] = r[j]
          rdotkepler[i,j] = rdot[j]
        end
        for j=1:6, k=1:6
          jac_kepler[(i-1)*6+j,i*7+k-1] = jac_21[j,k]
        end
        for j=1:n_body
